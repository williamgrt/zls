...
func (cfg *config) crash1(i int) {
	...
	// pass Make() the lastElectionTime persisted state.
	if cfg.saved[i] != nil {
...
// returns index.
// if retry==true, may submit the Data multiple
...
func (cfg *config) one(cmd interface{}, expectedServers int, retry bool) int {
	...
	for time.Since(t0).Seconds() < 10 {
		...
		if index != -1 {
			...
			// submitted our Data; wait a while for agreement.
			t1 := time.Now()
			for time.Since(t1).Seconds() < 2 {
				...
				if nd > 0 && nd >= expectedServers {
					// committed
					if cmd1 == cmd {
						// and it was the Data we submitted.
						return index
					}
				}
